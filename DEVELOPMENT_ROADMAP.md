...
- [ ] Treat every new integration as two tasks:
  - feature implementation
  - repeatable test coverage
- [ ] Keep local-only model loading as the default path for toolkit-native CLIP, DINOv2, and integrated VBench execution.
- [ ] Prefer incremental integrations:
  - first support summary-level outputs
  - then add per-video matching
  - then optimize UX and reporting
- [ ] Do not mark a milestone complete until the corresponding CLI path, config path, and smoke-test path all succeed.

## v0.1 Stabilization

Goal: fully test the current implementation one item at a time and close gaps in documentation, model setup, and regression coverage.
...
This subsection defines the shared fixture, base-pipeline expectations, and debugging order only. Module-specific smoke tests for native CLIP, native DINOv2, and integrated VBench belong to their dedicated subsections below.
...
- [ ] Only after the base pipeline passes, enable optional modules one at a time in this order:
  - native CLIP
  - native DINOv2
  - integrated official VBench
- [ ] If a later stage fails, treat the failure as scoped to the newly enabled module until proven otherwise.
- [ ] Use larger datasets only after the 2-sample base pipeline and optional-module smoke tests are already passing.

- [x] Freeze the current 2-sample fixture as the default smoke-test dataset and document its exact paths.
- [ ] Define the minimum smoke-test command for the base pipeline.
...
- [x] Record expected coverage behavior:
  - exactly 2 matched prompts with videos
  - remaining prompts may stay uncovered and should not fail the smoke test
- [x] Record expected base success criteria:
  - process exits successfully
  - output directory is created
  - the expected report files exist
  - there are exactly 2 evaluated rows in per-video outputs
  - there is exactly 1 model row in model summary for the tested model
- [x] Record acceptable smoke-test exclusions:
  - metric quality is not judged from 2 samples
  - cross-model comparison is out of scope
  - aggregate stability is out of scope
...
- [ ] Define the 2-sample CLIP smoke-test command.
...
- [ ] Record CLIP smoke-test pass criteria:
  - local CLIP checkpoint loads from cache
  - `clip_f.*`, `clip_t.*`, `drift_clip.*`, and `repetition_clip.*` columns are present when enabled
...
- [ ] Define the 2-sample DINOv2 smoke-test command.
...
- [x] Record DINOv2 smoke-test pass criteria:
  - local DINOv2 directory loads without network access
  - `dinov2_lc.*` and `drift_dinov2.*` columns are present when enabled
...
- [ ] Define the 2-sample VBench smoke-test command.
...
- [ ] Record VBench smoke-test pass criteria:
  - local VBench assets pass preflight
  - raw VBench output directory is created
  - merged `vbench.*` columns appear in summary output for requested dimensions
...
- [ ] Add a short troubleshooting section:
  ...
  - missing cache files
  - bad symlinks
  - proxy-related download failures
- [ ] Add a one-command verification checklist for local caches.
- [ ] Add a cache cleanup / refresh note for corrupted partial downloads.

## v0.2 Reporting and diagnostics
...
### v0.3.B VBench-Long-specific arguments
...
### v0.3.D Dependencies and preflight
...
- [ ] Perform real smoke tests for:
  - native CLIP
  - native DINOv2
  - each of the six integrated VBench dimensions
- [ ] Record current pass/fail status directly in this roadmap once each smoke test is completed.
