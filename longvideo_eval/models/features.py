...
class OpenAICLIPExtractor(FeatureExtractor):
    ...
    def __init__(
        ...
        cache_dir = Path(clip_cache_dir).expanduser() if clip_cache_dir is not None else None
...
class DINOv2Extractor(FeatureExtractor):
    ...
    Requires optional dependency: transformers and torch. The model checkpoint must be available
    locally or downloadable in the user's environment.
    """
...
def make_extractor(kind: str, **kwargs) -> FeatureExtractor:
    ...
    if kind in {"clip", "open_clip", "openclip"}:
        return build_clip_extractor(
            backend=kwargs.get("backend", "open_clip"),
    ...
    if kind in {"dinov2", "dino"}:
        return DINOv2Extractor(
            model_name=kwargs.get("model_name", "facebook/dinov2-base"),
            device=kwargs.get("device"),
        )
